  ...
  while (1)
  {
	...
	if(irq == 1){
		...
		if(stat & (1 << RX_DR)){
			...
			nrf24ReceiveData(dataR, sizeof(dataR));
			nrf24TransmitRXPayloadAsRX(0, rx_ack_pld, sizeof(rx_ack_pld));
			nrf24ClearRX_DR();
		}
		irq = 0;
	}
	...
	data = nrf24ConvertUint8ArrayToType(dataR, sizeof(dataR));
	...
	//char tmp1[40];
	//sprintf(tmp1, "\r\n| %d | \r\n", nrf24_r_reg(CONFIG, 1));

	HAL_UART_Transmit(&huart1, (uint8_t*)tmp, strlen(tmp), 100);

	//HAL_UART_Transmit(&huart1, tmp1, strlen(tmp1), 100);

	//for(uint8_t i = 0; i < sizeof(dataR); i++){
	//	dataR[i] = 0;
	//}
	//data = 0;
